...
float RoutePlanner::CalculateHValue(RouteModel::Node const *node) {
    ...
    float distance_to_end = node->distance(*end_node);
...
void RoutePlanner::AddNeighbors(RouteModel::Node *current_node) {
        ...
        for(auto neighbor : current_node->neighbors){
            // set the parent to be the current node for it's neighbors
            neighbor->parent = current_node;
            // set the h_value for each neighbor to be the distance to the end node
            neighbor->h_value = CalculateHValue(neighbor);
            // set the g_value for each neighbor to be the sum of the current node's g_value and the distance to the neighbor
            neighbor->g_value = current_node->g_value + current_node->distance(*neighbor);
            // add the neighbor to the open list
            open_list.emplace_back(neighbor);
            // set the neighbor's visited attribute to true
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    // reverse the path_found vector so that the start node is the first element
...
void RoutePlanner::AStarSearch() {
    ...
    while(!open_list.empty()){
        ...
        if(current_node == end_node){
            // if it is, construct the final path
            m_Model.path = ConstructFinalPath(current_node);
            return;
        }
        // if it is not, add the neighbors of the current node to the open list
